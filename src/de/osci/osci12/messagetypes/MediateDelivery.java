...
/**
 * <p>Mit dieser Klasse werden Nachrichtenobjekte für Abwicklungsaufträge
 ...
 * Erfolg der Operation (getFeedback()) und ggf. Inhaltsdaten vom Empfänger enthält.</p>
 *
 ...
 * @version 2.4.1
 *
 * @see de.osci.osci12.messagetypes.ResponseToMediateDelivery
 */
public class MediateDelivery extends OSCIRequest implements ContentPackageI
{
  //  private static Log log = LogFactory.getLog(MediateDelivery.class);
   ...
   * @param uriReceiver uri des Empfängers
   *
   * @see de.osci.osci12.common.DialogHandler
   * @throws NoSuchAlgorithmException undocumented
   * @throws URISyntaxException undocumented
   */
  public MediateDelivery(DialogHandler dh, Addressee addressee, String uriReceiver)
                  throws java.net.URISyntaxException,
                         NoSuchAlgorithmException
  {
    super(dh);
    messageType = MEDIATE_DELIVERY;
   ...
   */
  public ResponseToMediateDelivery send() throws IOException,
   ...
   * @see #send()
   */
  public ResponseToMediateDelivery send(OutputStream storeOutput, OutputStream storeInput)
                                 throws IOException,
                                        de.osci.osci12.OSCIException,
                                        NoSuchAlgorithmException
  {
  ...
  /**
   * Setzt den Betreff-Eintrag im Laufzettel. Der übergebene Parameter-String
                                  ...
                                  NoSuchAlgorithmException
  {
    ...
    String head = "<" + osciNSPrefix + ":ContentReceiver URI=\"" + uriReceiver.toString() + "\"></" + osciNSPrefix +
                  ":ContentReceiver>";

    if (messageId != null)
    {
      head += ("<" + osciNSPrefix + ":MessageId>" +
      de.osci.helper.Base64.encode(messageId.getBytes(Constants.CHAR_ENCODING)) + "</" + osciNSPrefix + ":MessageId>");
    ...
    osciH = new OsciH(HeaderTags.mediateDelivery.getElementName(), head);
