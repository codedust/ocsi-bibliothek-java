 ...
 * @version 0.9
 */
public class MultiFetchDelivery
{
  Intermed intermed;
   ...
   */
  public MultiFetchDelivery(String intermedURL)
                                                   ...
                                                   OSCIException
  {
    ...
    // Transmit the request and receive the response
    ArrayList<ResponseToFetchDelivery> listRsp= new ArrayList<>();
    ResponseToFetchDelivery rsp2FetchDel = fetchDel.send();
    listRsp.add(rsp2FetchDel);
    while(rsp2FetchDel.getFeedback()[0][1].startsWith("0"))
    {
      rsp2FetchDel = fetchDel.send();
      listRsp.add(rsp2FetchDel);
    }
  ...
  public static void main(String[] args)
  {
    try
    {
      MultiFetchDelivery scenario_1 = new MultiFetchDelivery(args[0]);
      ResponseToStoreDelivery responseStore = scenario_1.sendStoreDelivery();
       responseStore = scenario_1.sendStoreDelivery();
       responseStore = scenario_1.sendStoreDelivery();
       responseStore = scenario_1.sendStoreDelivery();
      System.out.println("\nResponseToStoreDelivery:\n" + responseStore.toString());

      ResponseToFetchDelivery responseFetchDel =null;// scenario_1.sendFetchDelivery(responseStore.getMessageId());
